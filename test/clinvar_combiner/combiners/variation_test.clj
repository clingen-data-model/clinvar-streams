(ns clinvar-combiner.combiners.variation-test
  (:require [clinvar-combiner.combiners.variation :as vc]
...
(test/deftest variation-list-to-compound-test
  (test/testing "Trivial case, nil"
    (let [variations nil
          expected []
          actual (vc/variation-list-to-compound variations)]
  ...
  (test/testing "Trivial case, nil variation"
    (let [variations [nil]
          ...
          expected []
          actual (vc/variation-list-to-compound variations)
          actual2 (vc/variation-list-to-compound variations2)]
      (test/is (= expected actual) (str "Expected did not match actual: "
                                        (clojure.data/diff expected actual)))
  ...
  (test/testing "General case, five layers"
    (let [variations [{:id "v1"
      ...
      (test/is (= expected actual) (str "Expected did not match actual: "
                                        (into [] (clojure.data/diff expected actual))))))
  (test/testing "General case, multiple root variations"
    (let [variations [{:id "v1"
                       :child_ids (json/generate-string ["v1-1"])}
                      {:id "v1-1"
                       :child_ids (json/generate-string ["v1-2"])}
                      ...
                      {:id "v2"
                       :child_ids (json/generate-string ["v2-1"])}
                      {:id "v2-1"
                       :child_ids (json/generate-string ["v2-2"])}
                      {:id "v2-2"}]
          expected [{:id "v1"
                     :child_variations
                     [{:id "v1-1"
      ...
      (test/is (= expected actual) (str "Expected did not match actual: "
                                        (into [] (clojure.data/diff expected actual))))))
  (test/testing "Error case, no root variations (cyclic relation)"
    (let [variations [{:id "v1"
                       :child_ids (json/generate-string ["v2"])}
                      {:id "v2"
                       :child_ids (json/generate-string ["v1"])}]]
      (test/is (thrown? ExceptionInfo (vc/variation-list-to-compound variations))
               (str "Expected exception: " variations))))
  )
...
(test/deftest validate-variation-tree
  (test/testing "Error case, nil variation"
    (let [variation nil]
      (test/is (thrown? ExceptionInfo
                        (vc/validate-variation-tree variation))
  ...
  (test/testing "General case, one variation"
    (let [variation {:id "v1" :subclass_type "SimpleAllele"}
      ...
      (test/is (vc/validate-variation-tree variation)
               (str "Expected validation success: " variation))
      (test/is (vc/validate-variation-tree variation2)
               (str "Expected validation success: " variation2))
      (test/is (vc/validate-variation-tree variation3)
  ...
  (test/testing "Error case, invalid Genotype child"
    (let [variation {:id "v1"
                     ...
                     [{:id "v1-1"
                       :subclass_type "Genotype"}]}]
      (test/is (thrown? ExceptionInfo (vc/validate-variation-tree variation))
               (str "Expected exception: " variation))))
  (test/testing "Error case, invalid Haplotype child"
    (let [variation {:id "v1"
      ...
      (test/is (thrown? ExceptionInfo (vc/validate-variation-tree variation))
               (str "Expected exception: " variation))
      (test/is (thrown? ExceptionInfo (vc/validate-variation-tree variation2))
