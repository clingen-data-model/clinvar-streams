...
(defn build-clinical-assertion
  ...
  (let [clinical-assertion (json/parse-string val true)
    ...
    (let [; variation archive
      ...
      (let [clinical-assertion (into {} (filter #(util/in? (first %) [:release_date]) (conj clinical-assertion (:content clinical-assertion))))
            updated-clinical-assertion
